export default class Deck{
    ...
    constructor()
    {
        this.newDeck = [["Two of Diamonds", "2" ], ["Three of Diamonds","3"], ["Four of Diamonds","4"],
        ...
        this.intCardNumber = 0;
        this.shuffledDeck = [];
        for (var i = 0; i < this.newDeck.length; i++)
        {
            this.shuffledDeck[i] = this.newDeck[i].slice();
        }
        this.CardsPicked = new Array(52).fill(false); 
    }

    Shuffle()
    {
        var rNum = Math.floor(Math.random() * 52);
        var intCard = 0;
        ...
        for (let i = 0; i < 52; i++)
            {
                do
                {
                    rNum = Math.floor(Math.random() * 52);
                    intCard = rNum;
                } while (this.CardsPicked[intCard]);
                this.CardsPicked[intCard] = true;
                this.shuffledDeck[i][0] = this.newDeck[intCard][0];
                this.shuffledDeck[i][1] = this.newDeck[intCard][1];
            }
    }
